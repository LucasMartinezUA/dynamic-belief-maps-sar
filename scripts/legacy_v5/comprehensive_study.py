#!/usr/bin/env python3
# scripts/comprehensive_study.py
"""
...
Planning modes:
  Geometric baseline:      pizza
  Greedy on prior:         static, static_3step
  Greedy on posterior:     dynamic, dynamic_3step

Usage:
  # Default: parallel execution using all cores
  python scripts/comprehensive_study.py

  # Sequential execution
  python scripts/comprehensive_study.py --single

  # Parallel with 4 workers
  python scripts/comprehensive_study.py -j 4

  # Run only specific studies
  python scripts/comprehensive_study.py --studies 5 6

  # Verbose subprocess logging
  python scripts/comprehensive_study.py --debug

  # Custom settings (50-100 runs for higher statistical power)
  python scripts/comprehensive_study.py --runs 100 --datasets 1 5 10 15 20 -j 8
"""
import argparse
import logging
import os
...
def run_single_job(cfg: StudyConfig) -> dict:
    ...
    if sim_mode.startswith("static"):
        initial_positions = [
            sim._grid_to_world(*sim._world_to_grid(wx, wy))
            for wx, wy in initial_positions
        ]

    sim.drone_positions = list(initial_positions)
    sim.drone_paths = [[pos] for pos in initial_positions]
    ...
    if sim_mode.startswith("static") and static_paths:
        sim._path_progress = [0.0] * len(static_paths)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Comprehensive SAR comparison study (reduced: 5 modes) with optional parallelization.",
